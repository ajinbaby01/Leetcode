...
func main() {
	fmt.Println(bookAllocation([]int{25, 46, 28, 49, 24}, 4))
}

func bookAllocation(books []int, students int) int {
	if len(books) < students {
		return -1
	}

	left, right := slices.Max(books), sumSlice(books)
	for left <= right {
...
// Check by assigning each student a book (since the allocation has to be contiguous)
func countStudents(books []int, mid int) int {
	student := 1
	pages := 0
